...
safeDiv x 0 = perform (\h -> failure h) ()
safeDiv x y = return (x `div` y)
-- END:exnex

safeHead :: (Exn :? e) => String -> Eff e Char
safeHead []    = perform (\h -> failure h) ()
...
symbol c = perform (\h -> satisfy h) (\input -> case input of
    (d:rest) | d == c -> Just (c, rest)
    _ -> Nothing)

digit :: (Parse :? e) => Eff e Int
digit = perform (\h -> satisfy h) (\input -> case input of
